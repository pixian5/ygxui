cat << 'ygxuiEOF' > /root/ygxui.sh
#!/bin/bash
XUI_USERNAME="7"
XUI_PASSWORD="7"
XUI_PORT="58964"
XUI_PATH="sbxbz478"
XUI_RESTORE_ON_BACKUP="1"
...
cronxui
echo "----------------------------------------------------------------------"
blue "x-ui-yg $(cat /usr/local/x-ui/v 2>/dev/null) 安装成功，自动进入 x-ui 显示管理菜单" && sleep 2
echo
x-ui
}

set_tls_paths(){
cert_file="/root/ygkkkca/cert.crt"
key_file="/root/ygkkkca/private.key"
if [[ -s $cert_file && -s $key_file ]]; then
help_msg=$(/usr/local/x-ui/x-ui setting -h 2>/dev/null)
if echo "$help_msg" | grep -q "webCertFile"; then
/usr/local/x-ui/x-ui setting -webCertFile ${cert_file} -webKeyFile ${key_file} >/dev/null 2>&1
elif echo "$help_msg" | grep -q "cert"; then
/usr/local/x-ui/x-ui setting -cert ${cert_file} -key ${key_file} >/dev/null 2>&1
fi
fi
}

restore_db(){
backup_db="/root/ygkkkca/x-ui-yg.db"
target_dir="/etc/x-ui-yg"
target_db="${target_dir}/x-ui-yg.db"
if [[ -s $backup_db ]]; then
yellow "检测到备份文件，正在自动恢复：${backup_db}"
mkdir -p "$target_dir"
if [[ x"${release}" == x"alpine" ]]; then
rc-service x-ui stop
else
...
chown -R root:root "$target_dir" >/dev/null 2>&1
green "备份恢复完成：${target_db}"
mv -f "$backup_db" "${backup_db}.$(date +%Y%m%d_%H%M%S)"
fi
}

xuiinstall(){
check_uninstall 1 || return
v6
echo "----------------------------------------------------------------------"
openyn
echo "----------------------------------------------------------------------"
serinstall
echo "----------------------------------------------------------------------"
if [[ "$XUI_RESTORE_ON_BACKUP" == "1" ]]; then
if [[ ! -f /root/ygkkkca/x-ui-yg.db ]]; then
yellow "未检测到备份文件，自动下载并放到 /root/ygkkkca/x-ui-yg.db"
mkdir -p /root/ygkkkca
tmp_db="/tmp/x-ui-yg.db"
curl -L -o "$tmp_db" -# --retry 2 --insecure https://raw.githubusercontent.com/pixian5/ygxui/main/x-ui-yg.db
if [[ -s "$tmp_db" ]]; then
cp -f "$tmp_db" /root/ygkkkca/x-ui-yg.db
green "备份下载成功"
else
red "备份下载失败"
fi
...
yellow "已在 /etc/x-ui-yg 路径导出备份文件x-ui-yg.db"
backup_src="/etc/x-ui-yg/x-ui-yg.db"
backup_dir="/root/ygkkkca"
backup_dst="${backup_dir}/x-ui-yg.db"
if [[ -f $backup_src ]]; then
mkdir -p "$backup_dir"
if [[ -e $backup_dst ]]; then
backup_dst="${backup_dir}/x-ui-yg.db.$(date +%Y%m%d_%H%M%S)"
fi
...
show_menu(){
if [[ -z $xui_autoinstall_checked ]]; then
check_status
if [[ $? == 2 ]]; then
xui_autoinstall_checked=1
xuiinstall
return
fi
fi
