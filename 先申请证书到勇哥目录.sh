...
show_cert_info() {
  ...
  echo "✅ cron：每日 UTC 20:00 执行自动续期"
}

# ===== 判断 acme.sh 里的证书是否存在且未过期 =====
acme_cert_valid() {
  # 需要 acme.sh 本地 fullchain + key 至少存在；有效期检查只看 fullchain
  ...
  # openssl -checkend N：若证书在未来 N 秒内过期则返回 1，否则返回 0
  # 这里用 N=0：只要“当前未过期”即视为有效
...
echo "✅ 已加入定时任务：每日 GMT 19:00 执行本脚本"
EOF
